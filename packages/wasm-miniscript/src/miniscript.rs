...
pub fn miniscript_from_string(script: &str, context_type: &str) -> Result<WrapMiniscript, JsValue> {
    match context_type {
        "tap" => Ok(WrapMiniscript::from(crate::error::wrap_err(Miniscript::<
            XOnlyPublicKey,
            Tap,
        >::from_str(script))?)),
        "segwitv0" => Ok(WrapMiniscript::from(crate::error::wrap_err(Miniscript::<
            PublicKey,
            Segwitv0,
        >::from_str(script))?)),
        "legacy" => Ok(WrapMiniscript::from(crate::error::wrap_err(Miniscript::<
            PublicKey,
            Legacy,
        >::from_str(script))?)),
